...
- `wifi_whitelist_enabled`: required. The JSON field is a plain boolean, but the handler decodes it into a `*bool` — a pointer so an explicit `false` (turning wifi off, an equally valid value) isn't indistinguishable from an omitted field under validator's `required` tag on a plain `bool`.
...
- `wifi_whitelist_enabled`: required. Same `*bool` reasoning as the single-store toggle above — the JSON field is a plain boolean, but the handler decodes it as a pointer so an explicit `false` is distinguishable from an omitted field.
...
- Server-side search, filter, or pagination on `GET /v1/stores` — it returns every store unconditionally (see List endpoint above); only query-driven narrowing is out of scope, not the list endpoint itself.
